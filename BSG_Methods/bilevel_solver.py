...
class BilevelSolverSyntheticProb:
    ...
    Attributes
        ...
        algo (str):                           Name of the algorithm to run ('bsg', 'darts', 'stocbio', 'sigd')
    ...
    def __init__(self, func, algo, algo_full_name, seed=0, ul_lr=5, ll_lr=0.1, max_iter=2000, normalize=False,\
    ...
    def main_algorithm(self): 
        ...
        for it in range(self.max_iter):
            ...
            if self.iprint >= 2:
                if self.true_func:
                    print("Algorithm: ",self.algo_full_name,' iter: ',it,' f_u: ',func_val_list[len(func_val_list)-1],' f: ',true_func_list[len(true_func_list)-1],' LL iter: ',self.llp_iters,' ul_lr: ',self.ul_lr,' ll_lr: ',self.ll_lr)    
                else:
                    print("Algorithm: ",self.algo_full_name,' iter: ',it,' f_u: ',func_val_list[len(func_val_list)-1],' LL iter: ',self.llp_iters,' ul_lr: ',self.ul_lr,' ll_lr: ',self.ll_lr)   
            
        if self.iprint >= 1:
            if self.true_func:
                print("Algorithm: ",self.algo_full_name,' f_u: ',func_val_list[len(func_val_list)-1],' f: ',true_func_list[len(true_func_list)-1],' time: ',time.time() - cur_time,' LL iter: ',self.llp_iters,' ul_lr: ',self.ul_lr,' ll_lr: ',self.ll_lr)    
            else:
                print("Algorithm: ",self.algo_full_name,' f_u: ',func_val_list[len(func_val_list)-1],' time: ',time.time() - cur_time,' LL iter: ',self.llp_iters,' ul_lr: ',self.ul_lr,' ll_lr: ',self.ll_lr)    
...
class BilevelSolverCL:
    ...
    def unpack_weights(self, model, grad):
        ...
        weights_list = np.hstack(unpacked_list)   #November2024 OLD: weights_list = np.hstack(np.array(unpacked_list))
    ...
    def true_function_value_constr(self, train_loader, training_task_loaders, test_loader, y, model, oldmodel, num_constr, batch, LLP_steps, true_func_list, accuracy_list, stream_training, stream_task_training_list, stream_validation): 
        """
        ...
        accuracy_list.append(self.accuracy_func(model(c_valid), u_valid))
        return true_func_list
    ...
    def main_algorithm(self):
    	...
    	true_func_list = []
    	time_list = []
    	accuracy_list = [] # November 2024 Update
        	...
        	# if i <= 2:
        		...
        		for cur_epoch in range(self.max_epochs):
        			...
        			if  cur_epoch == 0:
        				...
        				accuracy_list.append(float(self.func.accuracy_func(model(C), u)))    # November 2024 Update     
        			...
        			for batch in range(len(self.training_loaders[i])):
        				...
        				accuracy_list.append(self.func.accuracy_func(u_pred, u_valid))    # November 2024 Update 
        				...
        				if self.iprint >= 2:
        				        print("Algo: ",self.algo," cur_epoch: ",cur_epoch," task: ",i,' batch: ',batch,' f_u: ',func_val_list[len(func_val_list)-1],' time: ',time.time() - cur_time - time_tru_func_tot,' LL iter: ',self.llp_iters,' ul_lr: ',self.ul_lr,' ll_lr: ',self.ll_lr)    				
    	...
    	return [func_val_list, true_func_list, time_list, accuracy_list] # November 2024 Update
    ...
    def main_algorithm_avg_ci(self, num_rep=1):
        ...
        true_func_values = sol[1]
        times = sol[2]
        accuracy_values = sol[3] # November 2024 Update
        values_rep = np.zeros((len(values),num_rep))
        values_rep[:,0] = np.asarray(values)
        accuracy_values_rep = np.zeros((len(accuracy_values),num_rep)) # November 2024 Update
        accuracy_values_rep[:,0] = np.asarray(accuracy_values) # November 2024 Update
        if self.true_func:
            true_func_values_rep = np.zeros((len(true_func_values),num_rep))
            true_func_values_rep[:,0] = np.asarray(true_func_values)
        # Solve the problem num_rep-1 times
        for i in range(num_rep-1):
          ...
          if self.use_stopping_iter:
              values_rep[:,i+1] = np.asarray(sol[0])
              accuracy_values_rep[:,i+1] = np.asarray(sol[3]) # November 2024 Update
              if self.true_func:
                  true_func_values_rep[:,i+1] = np.asarray(sol[1])
          else:
              values_rep[:,i+1] = list(map(lambda x: self.piecewise_func(x,sol[2],sol[0]),times))
              accuracy_values_rep[:,i+1] = list(map(lambda x: self.piecewise_func(x,sol[2],sol[3]),times)) # November 2024 Update
              if self.true_func:
                  true_func_values_rep[:,i+1] = list(map(lambda x: self.piecewise_func(x,sol[2],sol[1]),times))
        values_avg = np.mean(values_rep, axis=1)
        values_ci = t.ppf(0.975,num_rep-1)*(np.sqrt(np.var(values_rep, ddof=1, axis=1))/np.sqrt(num_rep))
        accuracy_values_avg = np.mean(accuracy_values_rep, axis=1) # November 2024 Update
        accuracy_values_ci = t.ppf(0.975,num_rep-1)*(np.sqrt(np.var(accuracy_values_rep, ddof=1, axis=1))/np.sqrt(num_rep)) # November 2024 Update
        if self.true_func:
            true_func_values_avg = np.mean(true_func_values_rep, axis=1)
            true_func_values_ci = t.ppf(0.975,num_rep-1)*(np.sqrt(np.var(true_func_values_rep, ddof=1, axis=1))/np.sqrt(num_rep))
        else:
        ...
        return values_avg, values_ci, true_func_values_avg, true_func_values_ci, times, accuracy_values_avg, accuracy_values_ci # November 2024 Update
